...
app.post("/api/purchased-offers/:id/use-service", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const sessionsRes = await transaction.request().input("purchased_offer_id", sql.Int, id).input("service_index", sql.Int, service_index).query(`
      ...
      await transaction.request().input("id", sql.Int, serviceSession.id).input("remaining_sessions", sql.Int, newRemaining).query(`
...
// API سحب رصيد من العميل
app.post("/api/clients/:id/withdraw", async (req, res) => {
  const { id } = req.params;
  const { balance_type, amount, payment_method, notes, created_by } = req.body;
  ...
  const userCreator = created_by || "SYSTEM";
  ...
  try {
    ...
    try {
      // 1️⃣ جلب بيانات العميل
      const clientRes = await transaction.request().input("id", sql.Int, id).query("SELECT * FROM dbo.clients WHERE id = @id");
      ...
      // 3️⃣ التحقق من كفاية الرصيد
      if (currentBalance < parsedAmount) {
        await transaction.rollback();
        return res.status(400).json({
          message: `⚠️ الرصيد غير كافي!\n\nالرصيد الحالي: ${currentBalance.toFixed(2)} ج\nالمطلوب: ${parsedAmount.toFixed(2)} ج`,
        });
      }

      // 4️⃣ سحب الرصيد من العميل
      ...
      // 5️⃣ تسجيل المعاملة في جدول transactions
      await transaction
        .request()
        .input("client_id", sql.Int, id)
        .input("transaction_type", sql.NVarChar, "سحب رصيد")
        .input("amount", sql.Decimal(18, 2), -parsedAmount) // negative amount for withdrawal
        .input("balance_type", sql.NVarChar, balance_type)
        .input("payment_method", sql.NVarChar, payment_method || "نقدي")
        .input("created_by", sql.NVarChar, userCreator)
        .input("notes", sql.NVarChar, notes || `تم سحب ${parsedAmount.toFixed(2)} ج`).query(`
          INSERT INTO dbo.transactions 
            (client_id, transaction_type, amount, balance_type, payment_method, created_by, notes, created_at)
          VALUES 
            (@client_id, @transaction_type, @amount, @balance_type, @payment_method, @created_by, @notes, GETDATE());
        `);

      // 6️⃣ إيجاد user id من accounts بناءً على created_by (الاسم)
      ...
      // 7️⃣ إيجاد الشيفت المفتوح للمستخدم (إن وجد)
      if (userId !== null) {
        const shiftRes = await transaction.request().input("user_id", sql.Int, userId).query(`
          SELECT TOP 1 * FROM dbo.shifts 
        ...
        if (shiftRes.recordset && shiftRes.recordset.length > 0) {
          ...
          // 8️⃣ تسجيل العملية في جدول shift_operations (استعمل نفس توقيع الجدول لديك)
          await transaction
            .request()
            .input("shift_id", sql.Int, shiftId)
            .input("operation_type", sql.NVarChar, "سحب رصيد")
            .input("client_name", sql.NVarChar, client.name)
            .input("client_phone", sql.NVarChar, client.phone)
            .input("amount", sql.Decimal(18, 2), -parsedAmount) // negative amount for operation
            .input("payment_method", sql.NVarChar, payment_method || "نقدي")
            .input("balance_type", sql.NVarChar, balance_type)
            .input("description", sql.NVarChar, notes || `سحب ${balance_type}`).query(`
              INSERT INTO dbo.shifts_operation 
                (shift_id, operation_type, client_name, client_phone, amount, payment_method, balance_type, description, created_at)
              VALUES
                (@shift_id, @operation_type, @client_name, @client_phone, @amount, @payment_method, @balance_type, @description, GETDATE());
            `);

          // 9️⃣ تحديث total_deductions
          await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(18, 2), parsedAmount).query(`
              UPDATE dbo.shifts 
              SET total_deductions = ISNULL(total_deductions,0) + @amount
              WHERE id = @shift_id
            `);

          // 10️⃣ خصم من العمود المناسب في الشيفت (cash / visa / wallet / internal)
          // Map frontend payment_method values to columns:
          // - "نقدي" -> total_cash
          // - "POS" or "Visa" or "بطاقة" -> total_visa
          // - "تحويل" or "محفظة" or "Wallet" -> total_wallet
          // - "داخلي" -> total_internal

          const pm = (payment_method || "نقدي").toString();

          if (pm === "نقدي") {
            await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(18, 2), parsedAmount).query(`
                UPDATE dbo.shifts
                SET total_cash = ISNULL(total_cash,0) - @amount
                WHERE id = @shift_id
              `);
          } else if (pm === "POS" || pm === "Visa" || pm === "بطاقة") {
            await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(18, 2), parsedAmount).query(`
                UPDATE dbo.shifts
                SET total_visa = ISNULL(total_visa,0) - @amount
                WHERE id = @shift_id
              `);
          } else if (pm === "تحويل" || pm === "محفظة" || pm === "Wallet") {
            await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(18, 2), parsedAmount).query(`
                UPDATE dbo.shifts
                SET total_wallet = ISNULL(total_wallet,0) - @amount
                WHERE id = @shift_id
              `);
          } else if (pm === "داخلي") {
            await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(18, 2), parsedAmount).query(`
                UPDATE dbo.shifts
                SET total_internal = ISNULL(total_internal,0) - @amount
                WHERE id = @shift_id
              `);
          } else {
            // unknown payment method — do nothing to totals but still recorded in shift_operations
            console.warn("Unknown payment method when updating shift totals:", pm);
          }
        } // end if shift exists
      } // end if userId exists
      await transaction
        .request()
        .input("amount", sql.Decimal(18, 2), parsedAmount)
        .input("reason", sql.NVarChar, `سحب رصيد للعميل: ${client.name}`)
        .input("taken_by", sql.NVarChar, userCreator)
        .input("created_by", sql.NVarChar, userCreator).query(`
    INSERT INTO dbo.expenses (amount, reason, taken_by, created_by, created_at)
    VALUES (@amount, @reason, @taken_by, @created_by, GETDATE());
  `);
      await transaction.commit();

      return res.json({
        message: `✅ تم سحب ${parsedAmount.toFixed(2)} ج من ${balance_type} بنجاح`,
        new_balance: newBalance.toFixed(2),
      });
    } catch (err) {
      await transaction.rollback();
      console.error("Withdraw error:", err);
      return res.status(500).json({ message: "حدث خطأ أثناء تنفيذ السحب" });
    }
  } catch (err) {
...
app.get("/api/product-categories", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.product_categories ORDER BY name");
  ...
  try {
    ...
    try {
      const result = await transaction
        ...
        .input("location", sql.NVarChar, location || null).query(`
      ...
      if (stockQty > 0) {
        await transaction
          ...
          .input("created_by", sql.NVarChar, created_by || "النظام").query(`
...
app.put("/api/products/:id", async (req, res) => {
  const { id } = req.params;
  const { name, category_id, supplier_id, description, unit, purchase_price, selling_price, min_stock, max_stock, reorder_point, location, status } =
    req.body;
...
app.delete("/api/products/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.products WHERE id = @id");
...
app.post("/api/stock-movements", async (req, res) => {
  const { product_id, movement_type, quantity, reference_number, supplier_id, notes, created_by } = req.body;
  ...
  try {
    ...
    try {
      ...
      await transaction
        ...
        .input("created_by", sql.NVarChar, created_by).query(`
          INSERT INTO dbo.stock_movements 
      ...
      // تحديث رصيد المنتج
      await transaction.request().input("product_id", sql.Int, product_id).input("new_stock", sql.Int, newStock).query(`
          UPDATE dbo.products 
...
app.get("/api/products/:id/movements", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("product_id", sql.Int, id).query(`
...
app.get("/api/stock/stats", async (req, res) => {
  ...
  try {
    ...
    // عدد المنتجات
    const productsCount = await pool.request().query("SELECT COUNT(*) AS total FROM dbo.products WHERE status = 'active'");

    // المنتجات المنخفضة
    const lowStock = await pool.request().query("SELECT COUNT(*) AS total FROM dbo.products WHERE current_stock <= min_stock AND status = 'active'");

    // المنتجات النافذة
    const outOfStock = await pool.request().query("SELECT COUNT(*) AS total FROM dbo.products WHERE current_stock = 0 AND status = 'active'");

    // قيمة المخزون
    const inventoryValue = await pool
      .request()
      .query("SELECT ISNULL(SUM(current_stock * purchase_price), 0) AS total FROM dbo.products WHERE status = 'active'");

    // الحركات اليوم
    const todayMovements = await pool.request().query(`
...
app.get("/api/suppliers", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.suppliers ORDER BY id DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.get("/api/suppliers/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("SELECT * FROM dbo.suppliers WHERE id = @id");
...
app.post("/api/suppliers", async (req, res) => {
  ...
  try {
    ...
    const checkRes = await pool.request().input("phone", sql.NVarChar, phone).query("SELECT id FROM dbo.suppliers WHERE phone = @phone");
    ...
    const result = await pool
      ...
      .input("notes", sql.NVarChar, notes ? notes.trim() : null).query(`
...
app.delete("/api/suppliers/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.suppliers WHERE id = @id");
...
app.delete("/api/supplier-invoices/:id", async (req, res) => {
  ...
  try {
    ...
    try {
      const invoiceRes = await transaction
        .request()
        .input("id", sql.Int, id)
      ...
      await transaction.request().input("id", sql.Int, id).query("DELETE FROM dbo.supplier_invoices WHERE id = @id");
...
app.delete("/api/supplier-payments/:id", async (req, res) => {
  ...
  try {
    ...
    try {
      const paymentRes = await transaction
        .request()
        .input("id", sql.Int, id)
      ...
      await transaction.request().input("invoice_id", sql.Int, payment.invoice_id).input("amount", sql.Decimal(10, 2), payment.amount).query(`
          UPDATE dbo.supplier_invoices 
      ...
      await transaction.request().input("id", sql.Int, id).query("DELETE FROM dbo.supplier_payments WHERE id = @id");
...
app.get("/api/suppliers/stats", async (req, res) => {
  ...
  try {
    ...
    // عدد الموردين
    const suppliersCount = await pool.request().query("SELECT COUNT(*) AS total FROM dbo.suppliers");

    // المستحقات
    const totalDue = await pool.request().query("SELECT ISNULL(SUM(balance), 0) AS total FROM dbo.suppliers WHERE balance > 0");

    // المدفوعات الشهرية
    const monthlyPayments = await pool.request().query(`
    ...
    // الفواتير المعلقة
    const pendingInvoices = await pool.request().query("SELECT COUNT(*) AS total FROM dbo.supplier_invoices WHERE status = 'pending'");
...
app.post("/api/shifts/start", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("shift_type", sql.NVarChar, shift_type).query(`
...
app.post("/api/shifts/:id/operation", async (req, res) => {
  ...
  try {
    ...
    if (updateField) {
      await pool.request().input("shift_id", sql.Int, id).input("amount", sql.Decimal(10, 2), parseFloat(amount)).query(`
          UPDATE dbo.shifts 
...
app.get("/api/shifts/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("SELECT * FROM dbo.shifts WHERE id = @id");
...
app.post("/api/treasury/save-daily-report", async (req, res) => {
  ...
  try {
    ...
    const checkRes = await pool.request().input("date", sql.Date, date).query("SELECT id FROM dbo.daily_reports WHERE report_date = @date");
    ...
    const result = await pool
      ...
      .input("saved_by", sql.NVarChar, saved_by).query(`
...
app.get("/api/custodies", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.custodies ORDER BY created_at DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.post("/api/custodies/:id/return", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("id", sql.Int, id).input("returned_by", sql.NVarChar, returned_by).query(`
...
app.get("/api/expenses", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.expenses ORDER BY created_at DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.get("/api/treasury/balance", async (req, res) => {
  ...
  try {
    ...
    const totalIncome = parseFloat(reports.total_cash) + parseFloat(reports.total_wallet) + parseFloat(reports.total_visa);
...
app.put("/api/bookings/:id/reschedule", async (req, res) => {
  ...
  try {
    ...
    try {
      // جلب بيانات الحجز
      const bookingRes = await transaction.request().input("id", sql.Int, id).query("SELECT * FROM dbo.bookings WHERE id = @id");
      ...
      const conflictCheckRes = await transaction
        .request()
        ...
        .input("current_booking_id", sql.Int, id).query(`
  ...
  `);
      ...
      if (conflictCheckRes.recordset && conflictCheckRes.recordset.length > 0) {
        ...
        await transaction.rollback();
        return res.status(400).json({
          message: `⚠️ يوجد تعارض في جدول الدكتور!\n\nموعد موجود: ${formatTimeForConflict(conflict.start_time)} - ${formatTimeForConflict(
            conflict.end_time
  ...
  try {
    ...
    try {
      ...
      const result = await transaction
        ...
        .input("cancellation_date", sql.DateTime, new Date()).query(`
          UPDATE dbo.bookings 
          SET status = @status,
...
app.get("/api/clients/:id/purchased-offers", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("client_id", sql.Int, id).query(`
        SELECT 
          po.*, 
    ...
    for (let offer of result.recordset) {
      const sessionsRes = await pool.request().input("purchased_offer_id", sql.Int, offer.id).query(`
          SELECT * FROM dbo.offer_service_sessions 
...
app.post("/api/clients/:id/purchase-offer", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // جلب بيانات العرض
      const offerRes = await transaction.request().input("offer_id", sql.Int, offer_id).query("SELECT * FROM dbo.offers WHERE id = @offer_id");
      ...
      const clientRes = await transaction
        .request()
        .input("client_id", sql.Int, id)
      ...
      const existingOfferRes = await transaction.request().input("client_id", sql.Int, id).input("offer_id", sql.Int, offer_id).query(`
      ...
      if (existingOfferRes.recordset && existingOfferRes.recordset.length > 0) {
        ...
        for (let index = 0; index < services.length; index++) {
          ...
          const existingSessionsRes = await transaction
            .request()
            .input("purchased_offer_id", sql.Int, purchasedOfferId)
            .input("service_index", sql.Int, serviceIndex).query(`
              SELECT * FROM dbo.offer_service_sessions 
              WHERE purchased_offer_id = @purchased_offer_id AND service_index = @service_index
            `);
          ...
          if (existingSessionsRes.recordset && existingSessionsRes.recordset.length > 0) {
            ...
            await transaction.request().input("id", sql.Int, existingSession.id).input("additional_sessions", sql.Int, sessionsCount).query(`
                UPDATE dbo.offer_service_sessions 
            ...
            await transaction
              ...
              .input("remaining_sessions", sql.Int, sessionsCount).query(`
        ...
        const offerResult = await transaction
          .request()
          .input("client_id", sql.Int, id)
      ...
      await transaction
        ...
        .input("offer_name", sql.NVarChar, offer.name).query(`
          INSERT INTO dbo.transactions 
...
app.post("/api/purchased-offers/:id/use-session", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const sessionsRes = await transaction.request().input("purchased_offer_id", sql.Int, id).query(`
          SELECT * FROM dbo.offer_service_sessions 
      ...
      for (const session of sessionsRes.recordset) {
        if (session.remaining_sessions > 0) {
          await transaction.request().input("id", sql.Int, session.id).query(`
              UPDATE dbo.offer_service_sessions 
...
app.put("/api/clients/:oldId/change-id", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const checkNewIdRes = await transaction
        .request()
      ...
      const oldClientRes = await transaction.request().input("oldId", sql.Int, oldId).query("SELECT * FROM dbo.clients WHERE id = @oldId");
      ...
      const checkPhoneRes = await transaction
        .request()
      ...
      const newClientResult = await transaction
        .request()
        .input("newId", sql.Int, newId)
        ...
        .input("balance_old", sql.Decimal(10, 2), oldClient.balance_old || 0).query(`
      ...
      const updateTransactions = await transaction
        .request()
        .input("oldId", sql.Int, oldId)
        .input("newId", sql.Int, newId)
      ...
      const updateBookings = await transaction
        .request()
        .input("oldId", sql.Int, oldId)
        .input("newId", sql.Int, newId)
      ...
      await transaction.request().input("oldId", sql.Int, oldId).query("DELETE FROM dbo.clients WHERE id = @oldId");
...
app.get("/api/clients/:oldId/can-change-id/:newId", async (req, res) => {
  ...
  try {
    ...
    // التحقق من ID الجديد
    const checkNewId = await pool.request().input("newId", sql.Int, newId).query("SELECT id, name FROM dbo.clients WHERE id = @newId");

    // التحقق من العميل القديم
    const oldClient = await pool.request().input("oldId", sql.Int, oldId).query("SELECT * FROM dbo.clients WHERE id = @oldId");
...
app.get("/api/accounts", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT id, name, phone, role FROM dbo.accounts ORDER BY id DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.post("/api/accounts", async (req, res) => {
  ...
  try {
    ...
    const checkRes = await pool.request().input("phone", sql.NVarChar, phone).query("SELECT id FROM dbo.accounts WHERE phone = @phone");
...
app.put("/api/accounts/:id", async (req, res) => {
  ...
  try {
    ...
    const request = pool.request().input("id", sql.Int, id).input("name", sql.NVarChar, name);
...
app.delete("/api/accounts/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.accounts WHERE id = @id");
...
app.get("/api/categories", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT id, name, created_at FROM dbo.service_categories ORDER BY id DESC");
    return res.json(result.recordset || []);
  } catch (err) {
    console.error("Error fetching categories:", err.message);
...
app.post("/api/categories", async (req, res) => {
  ...
  try {
    ...
    const checkRes = await pool.request().input("name", sql.NVarChar, name.trim()).query("SELECT id FROM dbo.service_categories WHERE name = @name");

    if (checkRes.recordset && checkRes.recordset.length > 0) {
      return res.status(400).json({ message: "هذا القسم موجود مسبقاً" });
    }

    const result = await pool.request().input("name", sql.NVarChar, name.trim()).query(`
...
app.delete("/api/categories/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.service_categories WHERE id = @id");
...
app.post("/api/services", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("price", sql.Decimal(10, 2), parseFloat(price)).query(`
...
app.delete("/api/services/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.services WHERE id = @id");
...
app.get("/api/clients", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.clients ORDER BY id DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.get("/api/clients/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("SELECT * FROM dbo.clients WHERE id = @id");
...
app.get("/api/clients/:id/transactions", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("client_id", sql.Int, id).query(query);
...
app.post("/api/clients", async (req, res) => {
  ...
  try {
    ...
    const checkRes = await pool.request().input("phone", sql.NVarChar, phone).query("SELECT id FROM dbo.clients WHERE phone = @phone");
    ...
    try {
      const result = await transaction
        ...
        .input("balance_old", sql.Decimal(10, 2), parseFloat(balance_old) || 0).query(`
      ...
      for (const balance of balances) {
        if (balance.amount > 0) {
          await transaction
            ...
            .input("notes", sql.NVarChar, "تسجيل عميل جديد").query(`
...
app.delete("/api/clients/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.clients WHERE id = @id");
...
app.post("/api/clients/:id/charge-balance", async (req, res) => {
  ...
  try {
    ...
    const clientRes = await pool.request().input("id", sql.Int, id).query("SELECT * FROM dbo.clients WHERE id = @id");
    ...
    try {
      ...
      await transaction
        ...
        .input("payment_method", sql.NVarChar, payment_method || "نقدي")
        .input("created_by", sql.NVarChar, created_by)
        .input("notes", sql.NVarChar, `تم شحن ${parsedAmount.toFixed(2)} ج`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, payment_method, created_by, notes)
          VALUES (@client_id, @transaction_type, @amount, @balance_type, @payment_method, @created_by, @notes);
        `);
      ...
      if (userRes.recordset && userRes.recordset.length > 0) {
        ...
        if (shiftRes.recordset && shiftRes.recordset.length > 0) {
          ...
          await transaction
            ...
            .input("payment_method", sql.NVarChar, payment_method || "نقدي")
            .input("balance_type", sql.NVarChar, balance_type)
            .input("description", sql.NVarChar, `شحن ${balance_type}`).query(`
              INSERT INTO dbo.shift_operations 
              (shift_id, operation_type, client_name, client_phone, amount, payment_method, balance_type, description)
              VALUES 
              (@shift_id, @operation_type, @client_name, @client_phone, @amount, @payment_method, @balance_type, @description);
          ...
          if (updateField) {
            await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(10, 2), parsedAmount).query(`
                UPDATE dbo.shifts 
                SET ${updateField} = ${updateField} + @amount
                WHERE id = @shift_id
              `);
...
app.get("/api/clients/:clientId/bookings", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("client_id", sql.Int, clientId).query(`
        SELECT 
          b.id as booking_id,
          b.booking_date,
          b.start_time,
  ...
  try {
    ...
    try {
      ...
      await transaction
        ...
        .input("newToBalance", sql.Decimal(10, 2), newToBalance).query(`
          UPDATE dbo.clients 
      ...
      await transaction
        ...
        .input("created_by", sql.NVarChar, created_by)
        .input("notes", sql.NVarChar, `تم تحويل ${parsedAmount.toFixed(2)} ج من ${from_balance} إلى ${to_balance}`).query(`
      ...
      await transaction
        ...
        .input("created_by", sql.NVarChar, created_by)
        .input("notes", sql.NVarChar, `تم استقبال ${parsedAmount.toFixed(2)} ج من ${from_balance}`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
          VALUES (@client_id, @transaction_type, @amount, @balance_type, @created_by, @notes);
        `);
...
app.post("/api/clients/:id/transfer-to-client", async (req, res) => {
  ...
  try {
    ...
    const senderRes = await pool.request().input("id", sql.Int, id).query("SELECT * FROM dbo.clients WHERE id = @id");
    ...
    const receiverRes = await pool.request().input("phone", sql.NVarChar, target_phone).query("SELECT * FROM dbo.clients WHERE phone = @phone");
    ...
    try {
      ...
      await transaction
        ...
        .input("created_by", sql.NVarChar, created_by)
        .input("notes", sql.NVarChar, `تم تحويل ${parsedAmount.toFixed(2)} ج للعميل ${receiver.name} (${receiver.phone})`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
          VALUES (@client_id, @transaction_type, @amount, @balance_type, @created_by, @notes);
        `);
      ...
      await transaction
        ...
        .input("balance_type", sql.NVarChar, "رصيد أساسي")
        .input("created_by", sql.NVarChar, created_by)
        .input("notes", sql.NVarChar, `تم استقبال ${parsedAmount.toFixed(2)} ج من العميل ${sender.name} (${sender.phone})`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
          VALUES (@client_id, @transaction_type, @amount, @balance_type, @created_by, @notes);
        `);
...
app.get("/api/offers", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().query("SELECT * FROM dbo.offers ORDER BY created_at DESC");
    return res.json(result.recordset || []);
  } catch (err) {
...
app.delete("/api/offers/:id", async (req, res) => {
  ...
  try {
    pool = await sql.connect(cfg);
    const result = await pool.request().input("id", sql.Int, id).query("DELETE FROM dbo.offers WHERE id = @id");
...
app.get("/api/bookings/:doctorId/:date", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("doctor_id", sql.Int, doctorId).input("booking_date", sql.Date, date).query(`
        SELECT 
          b.*,
          c.name AS client_name,
          c.phone AS client_phone,
    ...
    // جلب الخدمات لكل حجز
    for (let booking of bookings) {
      const servicesRes = await pool.request().input("booking_id", sql.Int, booking.id).query(`
...
app.post("/api/bookings", async (req, res) => {
  ...
  try {
    ...
    const conflictCheck = await transaction
      ...
      .input("booking_date", sql.Date, booking_date)
      .input("start_time", sql.VarChar, formattedStartTime)
      .input("end_time", sql.VarChar, formattedEndTime).query(`
  ...
  `);
    ...
    if (conflictCheck.recordset && conflictCheck.recordset.length > 0) {
      ...
      await transaction.rollback();
      return res.status(400).json({
        message: `⚠️ يوجد تعارض في المواعيد!\n\nموعد موجود: ${formatTimeForConflict(conflictingBooking.start_time)} - ${formatTimeForConflict(
    ...
    if (!is_new_client && balance_type && shouldDeductBalance) {
      const clientRes = await transaction.request().input("client_id", sql.Int, client_id).query("SELECT * FROM dbo.clients WHERE id = @client_id");
    ...
    for (const service of services) {
      await transaction
        ...
        .input("price", sql.Decimal(10, 2), service.price).query(`
...
app.put("/api/bookings/:id/status", async (req, res) => {
  ...
  try {
    ...
    if (status === "بدأت" && previousStatus !== "بدأت" && previousStatus !== "انتهت") {
      ...
      if (isUnpaidBooking || (booking.balance_type && booking.balance_type !== "رصيد عروض")) {
        ...
        await transaction
          ...
          .input("notes", sql.NVarChar, `حجز موعد بتاريخ ${bookingDateStr} - الوقت: ${bookingTimeStr}`).query(`
            INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
            VALUES (@client_id, @transaction_type, @amount, @balance_type, @created_by, @notes);
          `);
        ...
        const shiftRes = await transaction.request().input("user_name", sql.NVarChar, currentUserName).query(`
        ...
        if (shiftRes.recordset && shiftRes.recordset.length > 0) {
          ...
          await transaction
            ...
            .input("booking_id", sql.Int, booking.id).query(`
              INSERT INTO dbo.shift_operations 
          ...
          await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(10, 2), totalPrice).query(`
              UPDATE dbo.shifts 
        ...
        try {
          ...
          if (offerData) {
            if (offerData.isFullOffer) {
              ...
              const sessionsRes = await transaction
                .request()
              ...
              const result = await transaction
                .request()
                .input("purchased_offer_id", sql.Int, offerData.offerId)
                .input("service_index", sql.Int, offerData.serviceIndex).query(`
                  UPDATE dbo.offer_service_sessions 
                  SET remaining_sessions = remaining_sessions - 1 
    ...
    // تحديث حالة الحجز
    const result = await transaction.request().input("id", sql.Int, id).input("status", sql.NVarChar, status).query(`
        UPDATE dbo.bookings 
        SET status = @status
        WHERE id = @id
      `);
...
app.delete("/api/bookings/:id", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await transaction.request().input("id", sql.Int, id).query("DELETE FROM dbo.bookings WHERE id = @id");
...
app.get("/api/bookings/by-creator", async (req, res) => {
  ...
  try {
    ...
    const request = pool.request().input("created_by", sql.NVarChar, created_by);
...
app.post("/api/clients/:id/phones", async (req, res) => {
  ...
  try {
    ...
    const mainPhoneRes = await pool.request().input("client_id", sql.Int, id).query("SELECT phone FROM dbo.clients WHERE id = @client_id");
...
app.put("/api/bookings/:id/price", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const bookingRes = await transaction.request().input("id", sql.Int, id).query("SELECT total_price FROM dbo.bookings WHERE id = @id");
      ...
      const result = await transaction.request().input("id", sql.Int, id).input("new_price", sql.Decimal(10, 2), finalPrice).query(`
          UPDATE dbo.bookings 
  ...
  try {
    ...
    try {
      ...
      const result = await transaction
        ...
        .input("start_time", sql.VarChar, formattedStartTime)
        .input("end_time", sql.VarChar, formattedEndTime)
        .input("duration", sql.Int, newDuration).query(`
          UPDATE dbo.bookings 
...
app.get("/api/accounts/:id/salary", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("id", sql.Int, id).query("SELECT fixed_salary FROM dbo.accounts WHERE id = @id");
  ...
  try {
    ...
    try {
      ...
      await transaction
        ...
        .input("duration", sql.Int, duration)
        .input("price", sql.Decimal(10, 2), servicePrice).query(`
      ...
      await transaction.request().input("booking_id", sql.Int, id).input("unpaid_amount", sql.Decimal(10, 2), servicePrice).query(`
          UPDATE dbo.bookings 
          SET notes = CASE 
...
app.post("/api/bookings/:id/add-service-instant", async (req, res) => {
  ...
  const { service_id, service_name, duration, price, balance_type, client_id, skip_shift_action } = req.body;
  ...
  try {
    ...
    try {
      ...
      const bookingRes = await transaction
        .request()
        .input("booking_id", sql.Int, bookingId)
      ...
      try {
        ...
        await transaction
          ...
          .input("duration", sql.Int, duration)
          .input("price", sql.Decimal(10, 2), priceAmount).query(`
      ...
      await transaction
        ...
        .input("notes", sql.NVarChar, `خصم فوري - خدمة إضافية: ${service_name} (حجز #${bookingId})`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
      ...
      if (!skip_shift_action) {
        ...
        const shiftRes = await transaction.request().query("SELECT TOP 1 id FROM dbo.shifts WHERE status = 'open' ORDER BY start_time DESC");
        ...
        if (shiftRes.recordset && shiftRes.recordset.length > 0) {
          ...
          await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(10, 2), priceAmount).query(`
              UPDATE dbo.shifts 
              SET total_internal = total_internal + @amount
              WHERE id = @shift_id
            `);
...
app.get("/api/bookings/:id/services", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request().input("booking_id", sql.Int, id).query(`
        SELECT * FROM dbo.booking_services 
        WHERE booking_id = @booking_id
        ORDER BY id
      `);
  ...
  try {
    ...
    try {
      ...
      await transaction
        ...
        .input("notes", sql.NVarChar, `دفع خدمات إضافية - الحجز رقم ${id}`).query(`
          INSERT INTO dbo.transactions (client_id, transaction_type, amount, balance_type, created_by, notes)
          VALUES (@client_id, 'دفع خدمات', @amount, @balance_type, @created_by, @notes);
        `);
      // ⭐ تسجيل العملية في الشيفت
      const shiftRes = await transaction.request().query(`
    SELECT TOP 1 id FROM dbo.shifts 
    WHERE status = 'open'
    ORDER BY start_time DESC
  `);
      ...
      if (shiftRes.recordset && shiftRes.recordset.length > 0) {
        ...
        await transaction
          ...
          .input("booking_id", sql.Int, id).query(`
      INSERT INTO dbo.shift_operations 
      (shift_id, operation_type, client_name, amount, payment_method, balance_type, description, booking_id)
      VALUES 
      (@shift_id, @operation_type, @client_name, @amount, @payment_method, @balance_type, @description, @booking_id);
    `);
        ...
        await transaction.request().input("shift_id", sql.Int, shiftId).input("amount", sql.Decimal(10, 2), paymentAmount).query(`
      UPDATE dbo.shifts 
      SET total_internal = total_internal + @amount
      WHERE id = @shift_id
    `);
      ...
      await transaction.request().input("booking_id", sql.Int, id).query(`
...
app.post("/api/session-details", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("doctor_role", sql.NVarChar, doctor_role).query(`
...
app.post("/api/bookings/:id/remove-service", async (req, res) => {
  ...
  try {
    ...
    const servicesResult = await pool.request().input("booking_id", sql.Int, id).query(`
    ...
    await pool.request().input("service_id", sql.Int, serviceToDelete.id).query("DELETE FROM booking_services WHERE id = @service_id");

    console.log("✅ تم حذف الخدمة من booking_services");

    // 5️⃣ تحديث السعر الإجمالي في جدول الحجوزات
    const bookingResult = await pool.request().input("id", sql.Int, id).query("SELECT total_price FROM bookings WHERE id = @id");
    ...
    if (balance_type && client_id) {
      const balanceField =
        balance_type === "رصيد أساسي"
          ? "balance_basic"
          : balance_type === "رصيد ليزر"
          ? "balance_laser"
          : balance_type === "رصيد بشرة"
          ? "balance_skin"
          : "balance_basic";
    ...
    try {
      ...
      if (tableCheck.recordset[0].table_exists > 0) {
        await pool
          ...
          .query(
            "INSERT INTO shift_transactions (type, amount, description, created_by, created_at) VALUES (@type, @amount, @description, @created_by, GETDATE())"
          );
...
app.post("/api/clients/:id/historical-charge", async (req, res) => {
  ...
  try {
    ...
    const clientResult = await pool.request().input("client_id", sql.Int, id).query("SELECT * FROM dbo.clients WHERE id = @client_id");
