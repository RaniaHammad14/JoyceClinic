...
`;
  ...
  function displayTransactions(transactions) {
    ...
    const html = transactions
      .map((transaction) => {
        ...
        const isHistorical = transaction.is_historical || (transaction.notes && transaction.notes.includes("[تاريخي]"));
            ...
            <div class="offer-services-section" style="margin-top: 20px; padding-top: 15px; border-top: 2px dashed #e0f2fe;">
              ...
              ${services
                .map((service, index) => {
                  ...
                  const serviceSession = offer.service_sessions?.find((s) => s.service_index == serviceIndex);
                ...
                <div class="service-item" style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: #f8fafc; border-radius: 8px; margin-bottom: 8px;">
                  ...
                  <button class="use-service-btn" 
                          ...
                          style="background: ${
                            canUseService ? "linear-gradient(135deg, #10b981 0%, #059669 100%)" : "#9ca3af"
                          }; color: white; border: none; padding: 8px 12px; border-radius: 6px; cursor: ${
                    canUseService ? "pointer" : "not-allowed"
                  }; font-size: 12px;"
  ...
  window.openBuyOfferModal = async (offerId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `✅ تم شراء العرض بنجاح!\n\nالعرض: ${offer.name}\nالسعر: ${offerPrice.toFixed(2)} ج\nالرصيد المتبقي: ${(
            currentOffersBalance - offerPrice
          ).toFixed(2)} ج`
        );
  ...
  window.openWithdrawModal = function (balanceType) {
    const withdrawModal = document.getElementById("withdrawModal");

    // Set client ID from URL
    document.getElementById("withdrawClientId").value = window.selectedClientId;

    // Set balance type
    document.getElementById("withdrawBalanceType").value = balanceType;

    // Reset fields
    document.getElementById("withdrawAmount").value = "";
    document.getElementById("withdrawPaymentMethod").value = "نقدي";
  ...
  // ===============================
  window.openWithdrawModal = function (balanceType) {
    const modal = $id("withdrawModal");
    const clientId = window.selectedClientId;

    if (!clientId) {
      alert("معرّف العميل غير موجود في الرابط");
      return;
    }

    // Fill modal fields
    $id("withdrawClientId").value = clientId;
    $id("withdrawBalanceType").value = balanceType;
    $id("withdrawAmount").value = "";
    $id("withdrawPaymentMethod").value = "نقدي";
    $id("withdrawNotes").value = "";
    $id("withdrawMessage").style.display = "none";
  ...
  window.confirmWithdraw = async function () {
    const clientId = $id("withdrawClientId").value;
    const balance_type = $id("withdrawBalanceType").value;
    const amountRaw = $id("withdrawAmount").value;
    const payment_method = $id("withdrawPaymentMethod").value;
    const notes = $id("withdrawNotes").value;

    // created_by fallback
    const created_by = localStorage.getItem("username") || localStorage.getItem("name") || localStorage.getItem("user") || "SYSTEM";

    const messageBox = $id("withdrawMessage");

    // Validation
    if (!clientId || !balance_type || !amountRaw) {
      messageBox.innerText = "يرجى ملء جميع البيانات المطلوبة";
    ...
    if (isNaN(amount) || amount <= 0) {
      messageBox.innerText = "الرجاء إدخال مبلغ صحيح";
      messageBox.style.display = "block";
      return;
    }
    ...
    // Send request
    try {
      const response = await fetch(`/api/clients/${clientId}/withdraw`, {
        method: "POST",
      ...
      if (!response.ok) {
        messageBox.innerText = data.message || "حدث خطأ أثناء السحب";
        messageBox.style.display = "block";
        return;
      }

      // Success
      messageBox.innerText = data.message || "تم السحب بنجاح";
      messageBox.style.display = "block";
      ...
      messageBox.innerText = "خطأ في الاتصال بالخادم";
      messageBox.style.display = "block";
    }
  };
  ...
  confirmDeleteBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const totalBalance =
        parseFloat(clientData.balance_basic || 0) +
  ...
  function displayBookings(bookings) {
    ...
    const html = bookings
      .map((booking) => {
        ...
        const servicesText = services.length > 0 ? services.map((s) => s.service_name || "خدمة").join(" و ") : "لا توجد خدمات";

        // ⭐ التحقق من نوع الحجز
        const isUnpaid =
          booking.notes &&
          (booking.notes.includes("[حجز مؤجل الدفع]") || booking.notes.includes("[حجز غير مدفوع]") || booking.notes.includes("[خدمات غير مدفوعة]"));
        const paymentType = isUnpaid ? "⚠️ دفع مؤجل" : "✅ مدفوع";
        const paymentColor = isUnpaid ? "#ff9800" : "#4caf50";

        return `
      <div class="visit-card" style="background: white; border: 2px solid ${
        isUnpaid ? "#ff9800" : "#e0e0e0"
  ...
  window.showServiceReport = async (bookingId, serviceId, serviceIndex) => {
    try {
      ...
      const serviceDetail = details.find((d) => d.service_id === serviceId && d.service_index === serviceIndex);
